...
export default function QcListPage() {
  ...
  // If Receive step is NOT enabled, we can create blind QC tasks
  const canCreateQc = !enabledSteps.includes(INBOUND_STEPS.RECEIVE);

  const columnDefs = useMemo<ColDef<InboundReceiptDto>[]>(
    () => [
      {
        field: "receiptNumber",
        headerName: t("inbound.qc.columns.qcCode", "QC Code"),
        pinned: "left",
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          if (status === ReceiptStatus.Receiving) {
            ...
            labelKey = "inbound.po.status.receiving"; // Will translate to something like "In Progress"
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          const qcTask = params.data as InboundReceiptDto;
          const isDraft = qcTask.status === ReceiptStatus.Receiving;
            ...
            <div className="flex items-center gap-1.5 h-full">
              {isDraft ? (
                <Button
                  ...
                  onClick={() => navigate(`/inbound/qc/${qcTask.id}`)}
                >
                  <ArrowRight className="size-3 mr-1" />
                  {t("inbound.qc.actions.continue", "Continue QC")}
                </Button>
              ) : (
                <Button
                  variant="ghost"
    ...
    return {
      getRows: async (params: IGetRowsParams) => {
        try {
          ...
          // Note: using receipts endpoint temporarily as placeholder per plan
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden bg-card border rounded-xl shadow-sm">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 border-b bg-secondary/10">
        <div>
          <h2 className="text-lg font-bold text-foreground">{t("inbound.dashboard.qc", "Quality Control")}</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            {t("inbound.dashboard.qcDescription", "Inspect received items")}
          </p>
        </div>
        {canCreateQc && (
          <Link to="/inbound/qc/create">
            <Button variant="default" size="sm" className="h-8 text-xs font-semibold cursor-pointer">
              <Plus className="size-3.5 mr-1" />
              {t("inbound.qc.actions.create", "Create QC Task")}
            </Button>
          </Link>
        )}
      </div>
