...
export default function PutawayListPage() {
  ...
  // If there are no preceding enabled steps (Receive or QC), allow Direct Putaway
  ...
  const columnDefs = useMemo<ColDef<InboundReceiptDto>[]>(
    () => [
      {
        field: "receiptNumber",
        headerName: t("inbound.putaway.columns.putawayCode", "Putaway Task"),
        pinned: "left",
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          const task = params.data as InboundReceiptDto;
          const isDraft = task.status === ReceiptStatus.Receiving;
            ...
            <div className="flex items-center gap-1.5 h-full">
              {isDraft ? (
                <Button
                  ...
                  onClick={() => navigate(`/inbound/putaway/${task.id}`)}
                >
                  <ArrowRight className="size-3 mr-1" />
                  {t("inbound.putaway.actions.continue", "Continue Putaway")}
                </Button>
              ) : (
                <Button
                  variant="ghost"
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden bg-card border rounded-xl shadow-sm">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 border-b bg-secondary/10">
        <div>
          <h2 className="text-lg font-bold text-foreground">{t("inbound.dashboard.putaway", "Putaway")}</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            {t("inbound.dashboard.putawayDescription", "Store items into warehouse locations")}
          </p>
        </div>
        {canCreatePutaway && (
          <Link to="/inbound/putaway/create">
            <Button variant="default" size="sm" className="h-8 text-xs font-semibold cursor-pointer">
              <Plus className="size-3.5 mr-1" />
              {t("inbound.putaway.actions.create", "Direct Putaway")}
            </Button>
          </Link>
        )}
      </div>
        ...
        <div className="w-full h-full">
          <AgGridReact
            ...
            onRowDoubleClicked={(e) => {
              if (e.data?.id) {
                navigate(`/inbound/putaway/${e.data.id}`);
              }
            }}
          />
        </div>
