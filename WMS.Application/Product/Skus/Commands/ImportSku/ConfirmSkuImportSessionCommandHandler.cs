...
namespace WMS.Application.Product.Skus.Commands.ImportSku.ConfirmSkuImportSession;
    ...
    : IRequestHandler<ConfirmSkuImportSessionCommand, ConfirmSkuImportSessionResponse>
{
        ...
        SkuImportSession session)
    {
        ...
        if (session.TotalRows == 0)
        {
            throw new AppException(
                409,
                "IMPORT_SESSION_HAS_NO_ROWS",
                "Import session has no rows.");
        }
    }
    ...
    private static void EnsureAllRowsAreValid(
        IReadOnlyList<SkuImportRow> rows)
    {
        var invalidRow = rows.FirstOrDefault(x => !x.IsValid);
        ...
        if (invalidRow is not null)
        {
            throw new AppException(
                409,
                "IMPORT_SESSION_HAS_INVALID_ROWS",
                $"Import row {invalidRow.RowNumber} is invalid.");
        }
    }
        ...
        IReadOnlyList<SkuImportRow> rows)
    {
        ...
        var rowMissingGoodsNature = rows.FirstOrDefault(x => string.IsNullOrWhiteSpace(x.GoodsNature));

        if (rowMissingGoodsNature is not null)
        {
            throw new AppException(
                409,
                "GOODS_NATURE_REQUIRED",
                $"Goods nature is required for row {rowMissingGoodsNature.RowNumber}.");
        }

        var rowMissingPrice = rows.FirstOrDefault(x => x.ReferencePrice is null);

        if (rowMissingPrice is not null)
        {
            throw new AppException(
                409,
                "REFERENCE_PRICE_REQUIRED",
                $"Reference price is required for row {rowMissingPrice.RowNumber}.");
        }
    }
