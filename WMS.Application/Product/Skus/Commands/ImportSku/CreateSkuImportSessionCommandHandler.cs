...
using WMS.Application.Product.Skus.Commands.ImportSku;
using WMS.Domain.Entities;
using WMS.Domain.Entities.Product;
using WMS.Domain.Interfaces;

namespace WMS.Application.Features.Skus.Import.CreateSkuImportSession;
    ...
    : IRequestHandler<CreateSkuImportSessionCommand, CreateSkuImportSessionResponse>
{
        ...
        IReadOnlySet<string> existingSkuCodes)
    {
        ...
        if (string.IsNullOrWhiteSpace(row.GoodsNature))
        {
            return RowValidationResult.Invalid(
                "GOODS_NATURE_REQUIRED",
                "Goods nature is required.");
        }

        if (row.ReferencePrice is null)
        {
            return RowValidationResult.Invalid(
