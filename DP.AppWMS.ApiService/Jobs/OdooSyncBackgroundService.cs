...
public class OdooSyncBackgroundService(
    IServiceProvider sp,
    IOptions<OdooConfig> config,
    ILogger<OdooSyncBackgroundService> log) : BackgroundService
{
    private readonly IServiceProvider _sp = sp;
    private readonly OdooSyncConfig _config = config.Value.Sync;
    private readonly ILogger<OdooSyncBackgroundService> _log = log;
        ...
        IServiceScope scope, CancellationToken ct)
    {
        ...
        _log.LogInformation("Odoo master sync: {Products} products, {Partners} partners", products, partners);
    }
}